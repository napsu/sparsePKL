!*************************************************************************
!*                                                                       *
!*     LMB-DCA  - Limited Memory Bundle DC Algorithm for nonsmooth       *
!*                nonconvex DC optimization                              *
!*                (last modified 29.08.2023 by Napsu)                    *
!*                                                                       *
!*     This file is specially modified to solve pairwise learning        *
!*     problems as a part of the sparsePKL software. The software is     *
...
!*     lmbdca  ! LMB-DCA
!*
MODULE lmbdca  ! Limited memory bundle DC algorithm

    USE r_precision, ONLY : prec  ! Precision for reals.
    IMPLICIT NONE

    ! MODULE lmbdca includes the following subroutines (S) and functions (F).
    PUBLIC :: &
        lmbdcm, &     ! S Limited memory bundle DC subroutine for nonsmooth
                      !   large-scale DC optimization. Contains:
        ...
        dlsr1, &      ! S Computing the search direction by limited
                      !   memory SR1 update.
        dca, &        ! S Computing the search direction using DCA.
...
CONTAINS
    ...
    !*     * SUBROUTINE lmbdcm *                                           *
    !*                                                                     *
    !*     Limited memory bundle subroutine for nonsmooth DC optimization. *
    !*                                                                     *
    !***********************************************************************
      
    SUBROUTINE lmbdcm(compute_ka,x,n,mc,f,nit,nfe,nge,iterm)
        ...
        USE initlmbdca, ONLY : &
            ...
            imet, &      ! Selection of the method:
                         !   imet = 0, lmbdca,
                         !   imet = 1, original dca with lmbm.                          
        ...
        USE obj_fun, ONLY : &
            ...
            myf1, &      ! Computation of the value of the function f1.
            myf2, &      ! Computation of the value of the function f2.
            myg1, &      ! Computation of the subgradient of the function f1.
            myg2         ! Computation of the subgradient of the function f2.
...
!The following lines define the signature of compute_ka for F2PY:
!f2py   integer :: mmpy,nnpy
!f2py   real(kind=prec) :: pppy(mmpy),aapy(nnpy)
!cf2py  compute_ka(pppy,aapy,mmpy,nnpy)
        ...
        ! Local Scalars
        REAL(KIND=prec) :: &
            forig, &     ! The value of f at the starting point.  
        ...
        INTEGER :: i, &
            ndca, &
            nitdca, &    ! Iteration counter for dca step.
            nf1dca,nf2dca, & ! Evaluation counters for dca step.
            ng1dca,ng2dca, & ! Evaluation counters for dca step.
        ...
        REAL(KIND=prec), PARAMETER :: &
            ...
            tmin    = 1.0E-32_prec, &  ! Minimum stepsize.
        ...
        CALL myf(compute_ka,n,x,f,iterm)
        forig = f
        PRINT*,'f original:  ',f
        nfe = nfe + 1

        IF (imet == 1) THEN ! Pure DCA 

            PRINT*,'Optimization with DCA.'
            xk = x
            CALL dca(compute_ka,n,x,f,iterm,nit,nf1,nf2,ng1,ng2)
            IF (iterm == 0) iterm = 1
            PRINT*,'nit =',nit,'nf1 =',nf1,'nf2 =',nf2,'ng1 =',ng1,'ng2 =',ng2,'f original =',forig,'f =',f,'iterm = ',iterm
            RETURN
        END IF

        PRINT*,'Optimization with LMB-DCA.'
        CALL myg(compute_ka,n,x,g,iterm)
        nge = nge + 1
        ...
        iteration: DO
            ...
            ELSE 
                ...
                IF (nres == 1) THEN
                    ...
                    IF (ncres > maxnrs) THEN
                        PRINT*,'Too many restarts. Applying DCA.'
                        ndca = ndca + 1
                        if (iters == 0) then
                            xk = xo ! we have had a null step, so xo is the current iteration point and x is an auxiliary point.
                            x = xo
                        else
                            xk=x
                        end if
                        CALL dca(compute_ka,n,x,f,iterm,nitdca,nf1dca,nf2dca,ng1dca,ng2dca)
                        nf1=nf1+nf1dca
                        nf2=nf2+nf2dca
                        ng1=ng1+ng1dca
                        ng2=ng2+ng2dca
                        IF (iterm == 0) THEN
                            iterm = 9 
                            EXIT iteration
                        ELSE IF (ABS(fo-f) <= 1.0E-10_prec) THEN
                            iterm = 9 
                            EXIT iteration
                        ELSE IF (iterm < 0) THEN ! failure
                            EXIT iteration
                        END IF
                        
                        CALL myg(compute_ka,n,x,g,iterm) 
                        nge = nge + 1
                        iters = 1 ! serious step
                        iterm = 0 
                        nnk = 0 
                    END IF
                    ...
                    CYCLE iteration
                END IF
                PRINT*,'Crescent direction. Applying DCA.' 
                ndca = ndca + 1
                if (iters == 0) then
                    xk = xo ! we have had a null step, so xo is the current iteration point and x is an auxiliary point.
                    x = xo
            ...
            IF (iters == 0) THEN
                IF (ABS(xnorm - pxnorm) <= small) THEN
                    ...
                    IF (neps > maxeps) THEN
                        ...
                        xk = xo ! we have had a null step, so xo is the current iteration point and x is an auxiliary point.
                        x = xo
                        PRINT*,'Does not converge. Applying DCA.'
                        CALL dca(compute_ka,n,x,f,iterm,nitdca,nf1dca,nf2dca,ng1dca,ng2dca)
                        nf1=nf1+nf1dca
                        nf2=nf2+nf2dca
                        ng1=ng1+ng1dca
                        ...
                        nge = nge + 1
                        iters = 1 ! serious step
                        iterm = 0 ! maybe we should check if iterm = -3 i.e. failure
                        nnk = 0 
                    END IF
            ...
            ELSE
                PRINT*,'Difficulties with the initial step size. Applying DCA.'
            ...
            IF (inma == 0) THEN ! Line search with directional derivatives which allows null steps
                theta = one
                IF (dnorm > lengthd) THEN
                    theta=lengthd/dnorm
                END IF
      
                CALL lls(compute_ka,n,x,g,d,xo,t,fo,f,p,alfn,tmin,dnorm,xnorm, &
            ...
            ELSE ! Nonmonotone line search with directional derivatives which allows null steps
                ...
                CALL nmlls(compute_ka,n,x,g,d,xo,t,fo,f,fold,p,alfn,tmin,dnorm,xnorm, &
                    theta,epsl,epsr,eta,iters,inewnma,nfe,nge,nnk,iterm)

            END IF
            
            IF ((iters == 0 .and. nnk > 10) .or. iters == 2) THEN !DCA if no descent step or null step condition is satisfied at t
                ndca = ndca + 1
                xk = xo ! we have had a null step, so xo is the current iteration point and x is an auxiliary point.
                x = xo
                PRINT*,'Too many null steps. Applying DCA.'
                CALL dca(compute_ka,n,x,f,iterm,nitdca,nf1dca,nf2dca,ng1dca,ng2dca)
                nf1=nf1+nf1dca
                nf2=nf2+nf2dca
                ng1=ng1+ng1dca
            ...
            IF (tolf2 >= 0) THEN
                IF (tolf2 > f/fo - 1.0 .AND. iters == 1) THEN
                    iterm = 3
                    EXIT iteration
                END IF
            END IF
...
900 CONTINUE
    ...
    PRINT*,'nit =',nit,'ndca =',ndca,'nf =',nfe,'nf1 =',nfe+nf1,'nf2 =',nfe+nf2,'ng =',nge,'ng1 =',nge+ng1,'ng2 =',&
 &       nge+ng2,'f original =',forig,'f =',f,'wk =',xnorm,'qk =',half*gnorm+alfv,'iterm = ',iterm
...
END SUBROUTINE lmbdcm

!************************************************************************
!*                                                                      *
!*     * SUBROUTINE dca *                                               *
!*                                                                      *
!*     Computation of x_k+1 by means of DCA                             *
!*                                                                      *
!************************************************************************

SUBROUTINE dca(compute_ka,n,x,f,iterm,nit,nf1,nf2,ng1,ng2)

    USE param, ONLY : one,large  ! Parameters.
    USE initlmbdca, ONLY : &
        imet, &       ! selection of the method LMBDCA or DCA
        mcinit, &     ! Initial maximum number of stored corrections.
        xk, &         ! Saved point xk.
        f2, &         ! Saved value for second component fuction at xk.
        g2, &         ! Saved subgradient for f2 at xk.
        maxit         ! Max number of iterations in dca 
    USE obj_fun, ONLY : &
        myf1, &       ! Computation of the first component function.
        myf2,myg2     ! Computation of the value and the subgradient of
                      ! the second component function.
    USE solvedca, ONLY : &
        dclmb         ! limited memory bundle method used to solve dca problem.
    ...
    INTEGER, INTENT(OUT) :: &
        nit, &          ! no of dca iterations.
        nf1, &          ! no of f1 calls in dca.
        nf2, &          ! no of f2 calls in dca.
        ng1, &          ! no of f1 subgradient calls in dca.
        ng2, &          ! no of f2 subgradient calls in dca.        
        iterm           ! Cause of termination:
                        !   0  - Everything is ok.
                        !   5  - Iterations in DCA are greater than maxit.
                        !  -3  - Failure in function calculations
            
    REAL(KIND=prec) :: &
        f1, f2prev,&
        fold, &         ! previous f value.
        flmbm, &        ! f value at the beginning of DCA
        fdiff, &        ! difference between flmbm and f
        eps             ! stopping tolerance.
    REAL :: &
        time1
        
    INTEGER :: &
        nit_lmbm, &     ! no of iterations in one round of lmbm.
        nf1tmp, &       ! number of f1 calls in lmbm.
        ng1tmp, &       ! number of f1 subgradient calls in lmbm.
        mc              ! number of corrections for lmbm.
    INTEGER :: turha = 0 !
    REAL(KIND=prec) :: &
        g2tmp(n)
    EXTERNAL compute_ka
    ...
    eps   = 1.0E-8_prec    
    time1 = 0.0        ! No time limit for dca   
    fold  = large
    iterm = 0
    nit   = 0
    nit_lmbm = 0
    ...
    nf1 = nf1 + 1
    f = f - f2 
    print*,'f before dca',f 
    flmbm = f
    
    ! LMBM for solving the DCA problem
    DO WHILE (nit < maxit)
        nit = nit+1
        fold = f
        
        CALL dclmb(compute_ka,x,n,mc,f,nit_lmbm,nf1tmp,ng1tmp,iterm) ! We only compute values for the first component function in each iteration of lmbm.
        nf1 = nf1 + nf1tmp
        ng1 = ng1 + ng1tmp
        
        ! Stopping dca step
        IF ((fold - f)/(ABS(fold)+one) < eps) THEN
            print*,"No progress with DCA. "
            CALL myf2(n,x,f2,iterm)
            CALL myf1(compute_ka,n,x,f,iterm)
            f = f - f2
            print*,"Current f = ",f, " previous f = ",flmbm
            RETURN 
        END IF
        ...
        f = f - f2 ! value of the objective at current point
        fdiff = (flmbm - f)/(ABS(flmbm)+one)
        if (imet == 0) then ! it is enough to find descent step with LMBDCA
            if (fdiff > eps) then ! 
                print*,"Descent step found with DCA. "
                iterm = 10 
                RETURN
            end if
        end if    
        ...
        !Stopping dca step 2
        IF ((fold - f)/(ABS(fold)+one) < eps) THEN
        !IF (ABS(fold - f)/(ABS(fold)+one) < eps) THEN
            print*,"No progress with DCA.",fold - f, fold, f,nit
            RETURN 
        END IF
    END DO

    iterm = 10
    print*,'f final dca ',f,' f original dca ',flmbm
...
SUBROUTINE lls(compute_ka,n,x,g,d,xo,t,fo,f,p,alfn,tmin,dnorm,wk,theta,epsl,epsr,&
...
!        CALL myg(compute_ka,ppy,n,x,g,iterm)
        CALL myg(compute_ka,n,x,g,iterm)
        nge = nge + 1
        IF (iterm /= 0) RETURN
...
!    SUBROUTINE nmlls(compute_ka,ppy,n,x,g,d,xo,t,fo,f,fold,p,alfn,tmin,dnorm,wk,theta,epsl,epsr,&
!        eta,iters,inewnma,nfe,nge,nnk,iterm)
        SUBROUTINE nmlls(compute_ka,n,x,g,d,xo,t,fo,f,fold,p,alfn,tmin,dnorm,wk,theta,epsl,epsr,&
            eta,iters,inewnma,nfe,nge,nnk,iterm)
