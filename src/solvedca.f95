!*************************************************************************
!*                                                                       *
!*     solve-dca  - Limited Memory Bundle Method for solving the         *
!*                  underlying convex problem                            *
!*                                                                       *
!*     (last modified 29.08.2023 by Napsu)                               *
!*                                                                       *
!*     This code is part of the sparsePKL software covered by the        *
!*     MIT license.                                                      *                                                
!*                                                                       *
!*************************************************************************
!*
!*     Modules included:
!*
!*     solvedca  ! LMBM for solving DCA problem.
!*
MODULE solvedca  
    ...
    ! MODULE solvedca includes the following subroutines (S) and functions (F).
    PUBLIC :: &
        dclmb, &      ! S Limited memory bundle subroutine for solving DCA problem. 
                      ! Contains:
...
CONTAINS
    ...
    !*     * SUBROUTINE dclmb *                                            *
    !*                                                                     *
    !*     Limited memory bundle subroutine for solving the DCA problem    *
    !*                                                                     *
    !***********************************************************************
      
    SUBROUTINE dclmb(compute_ka,myx,n,mc,f,nit,nfe,nge,iterm)
        ...
        USE initlmbdca, ONLY : &
            ...
            iprint=>ipdca, & ! Printout specification for dca (see initializat for more details).
            ...
            mit=>mitdca, & ! Maximun number of iterations for dca.
            mfe=>mfedca  ! Maximun number of function evaluations for dca.
        USE obj_fun, ONLY : &
            dcaf, &      ! Computation of the value of the DCA objective. 
            dcag         ! Computation of the subgradient of the DCA objective. 
        USE subpro, ONLY : &
        ...
        ! Scalar Arguments
        integer, intent(in) :: n
        real(kind=prec), intent(inout)  :: myx(n)
        ...
        CALL dcaf(compute_ka,n,myx,f,iterm)
        CALL dcag(compute_ka,n,myx,g,iterm)
        nfe = nfe + 1
        nge = nge + 1
        ...
        CALL dobun(n,na,mal,myx,g,f,ax,ag,af,iters,ibun)
        ...
        iteration: DO
            ...
            CALL rprint(n,nit,nfe,nge,myx,f,xnorm,half*gnorm+alfv,iterm,iprint)
            ...
            CALL tinit(n,na,mal,myx,af,ag,ax,ibun,d,f,p,t,tmax,tmin, &
                eta,iters,iterm)
            ...
            IF (inma == 0) THEN! Line search with directional derivatives which allows null steps
                ...
                CALL lls(compute_ka,n,myx,g,d,xo,t,fo,f,p,alfn,tmin,dnorm,xnorm, &
            ...
            ELSE ! Nonmonotone line search with directional derivatives which allows null steps
                ...
                CALL nmlls(compute_ka,n,myx,g,d,xo,t,fo,f,fold,p,alfn,tmin,dnorm,xnorm, &
...
SUBROUTINE lls(compute_ka,n,x,g,d,xo,t,fo,f,p,alfn,tmin,dnorm,wk,theta,epsl,epsr,&
    ...
    lls_iteration: DO
        ...
        CALL dcaf(compute_ka,n,x,f,iterm)
        ...
        CALL dcag(compute_ka,n,x,g,iterm)
        nge = nge + 1
        IF (iterm /= 0) RETURN
...
SUBROUTINE nmlls(compute_ka,n,x,g,d,xo,t,fo,f,fold,p,alfn,tmin,dnorm,wk,theta,epsl,epsr,&
    ...
    USE param, ONLY : zero,half,one
    USE obj_fun, ONLY : &
        dcaf,dcag   ! Computation of the value and the subgradient of
                    ! the DCA objective function.
    USE subpro, ONLY : &
        scsum       ! Sum of a vector and the scaled vector.
    USE initlmbdca, ONLY : &
